...
class DynamicBatchCallback(TrainerCallback):
    """
    Multiply `gradient_accumulation_steps` by the factors you supply
    whenever `global_step` reaches a trigger step.
    `schedule` is a list of tuples (trigger_step, decay_factor).
    """
    def __init__(self, schedule: list[tuple[int, float]], init_accum: int):
        self.schedule = sorted(schedule)          # [(step, 0.5), (step, 0.25)…]
        self.init_accum = init_accum

    def on_step_begin(self, args, state, control, **kwargs):
        # Figure out how many decays have already fired
        accum = self.init_accum
        init_accum = self.init_accum
        for step, factor in self.schedule:
            #raise ValueError(self.schedule)
            if state.global_step >= step:
                accum = max(1, int(init_accum * factor))
            else:
                break
...
class CustomTrainer(Trainer):
        ...
        def __init__(self, 
                    batch_size_scheduler= {'decay_at': [0.5], 'B_decay_amt': [1.0]},
                    momentum: float = 0.9,
            ...
            if self.args.max_steps > 0:                     # most online runs use this
                total_steps = self.args.max_steps
            else:                                           # fallback: derive from data
                raise ValueError("max_steps must be set in TrainingArguments. ")

            batch_size_scheduler = self.step_decay_schedule_dict
            schedule = []
            for frac, factor in zip(batch_size_scheduler['decay_at'],
                                    batch_size_scheduler['B_decay_amt']):
                trigger_step = int(frac * total_steps)
                schedule.append((trigger_step, factor))

            #raise ValueError("Stopping here to check schedule: ", schedule)
            # ---- register the callback ----
            init_accum = self.args.gradient_accumulation_steps
            self.add_callback(DynamicBatchCallback(schedule, init_accum))
            #self.add_callback(AdamStateReset(self.args.max_steps // 2))
            #self.reset_step = self.args.max_steps // 2
...
def main():
    ...
    if len(sys.argv) > 1:
            ...
            temp_path = 'outputs/nn_hf/aug_20_3/'  # default value
    ...
    # look at the step_decay_schedule
    # to determine the micro_bs
    # it should be set to the smallest batch size that is used in the step_decay_schedule
    # and the initial gradient accumulation steps should be set to h_dict["B"] // micro
    BSs = {h_dict["B"]} # set to the batch size from h_dict
    if "step_decay_schedule" in h_dict:
        if "B_decay_amt" in h_dict["step_decay_schedule"]:
            # if B_decay_amt is present, use it to determine the micro_bs
            for decay_amt in h_dict["step_decay_schedule"]["B_decay_amt"]:
                BSs.add(int(h_dict["B"] * decay_amt))
    micro_bs = min(3072, min(BSs))  # Get the smallest batch size from the set
    #micro_bs = 60 #12
    init_grad_accum = h_dict["B"] // micro_bs
    if init_grad_accum <= 0:
        raise ValueError(f"Batch size {h_dict['B']} is too small for micro batch size {micro_bs}.")
    print(f"Using micro batch size: {micro_bs}, initial gradient accumulation steps: {init_grad_accum}")
    ...
    args = TrainingArguments(
        ...
        gradient_accumulation_steps=  init_grad_accum,  # Set initial gradient accumulation steps
