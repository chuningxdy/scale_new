...
class CustomTrainer(Trainer):
        ...
        def _compute_weight_norm(self):
            ...
            for p in self.model.parameters():
                total_norm_sq += p.data.float().norm() ** 2
        ...
        def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
            ...
            print(f"  Weight norm at step {self.state.global_step}: {weight_norm:.4f}")
        ...
        def plot_eval_metrics(self):
            """Create a plot of evaluation loss and weight norm over steps"""

            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 10), sharex=True)

            ax1.plot(self.eval_steps, self.eval_losses, marker='o')
            ax1.set_title('Evaluation Loss Over Training')
            ax1.set_ylabel('Loss')
            ax1.grid(True)

            ax2.plot(self.eval_steps, self.weight_norms, marker='o', color='orange')
            ax2.set_title('Total Parameter Weight Norm Over Training')
            ax2.set_xlabel('Training Steps')
            ax2.set_ylabel('Weight Norm (L2)')
            ax2.grid(True)
