...
def EM_nqs_from_cfg_six(nqs_cfg, working_file_path = None):
    if nqs_cfg.h.lr_schedule in ["optimized"]:
        return EM_nqs_from_cfg_six_optimized(nqs_cfg, working_file_path)
...
def compute_risk_trajectory_opt_lr(B, K_target, learning_rate, 
    ...
    for stage in range(n_stages):
        ...
        if steps_in_prev_stage == 0:
            ...
            trys = -1
            while new_risk_at_attempt < old_risk_at_attempt - 0.05: #0.001: !!!
                trys += 1
                if verbose:
...
def EM_nqs_from_cfg_six_optimized(nqs_cfg, working_file_path = None):
