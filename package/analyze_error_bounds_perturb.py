"""
Analyze error bounds produced by error_bound_perturb.py.
  - Box plot of inferred parameters across trials
  - Actual vs predicted plot with error bars on test predictions
  - IsoFLOP / IsoToken curves with error bound shading

Usage:
    python analyze_error_bounds_perturb.py
"""
...
fig.suptitle('Parameter Estimates Across Trials (Perturbation)', fontweight='bold', y=1.02)
plt.tight_layout()
...
# Error bars: asymmetric around trial mean
...
ax.set_title('Actual vs Predicted (Perturbation)', fontweight='bold')
ax.legend()
ax.set_aspect('equal')
...
# Load full dataset for type/C info
all_rows = []
...
# Build results with full-fit predictions + per-trial bounds
results = []
for row in all_rows:
    split = row['split']
